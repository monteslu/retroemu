#!/bin/bash
# Build script for gearcoleco (ColecoVision)
# The Makefile is in platforms/libretro subdirectory

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
BUILD_DIR="$PROJECT_DIR/build/gearcoleco"
...
if [ ! -d "$BUILD_DIR/src" ]; then
  echo "Cloning gearcoleco from https://github.com/drhelius/Gearcoleco.git..."
  git clone --depth 1 "https://github.com/drhelius/Gearcoleco.git" "$BUILD_DIR/src"
fi

# Build the core with emscripten
echo "Building gearcoleco..."
cd "$BUILD_DIR/src/platforms/libretro"
emmake make platform=emscripten clean 2>/dev/null || true
emmake make platform=emscripten -j"$(nproc)"

# Find the built output
CORE_LIB=$(find . -maxdepth 2 \( -name "gearcoleco*.a" -o -name "gearcoleco*_libretro_emscripten.bc" \) | head -1)

if [ -z "$CORE_LIB" ]; then
  echo "Error: Could not find compiled core library for gearcoleco"
  exit 1
fi
...
echo "Linking gearcoleco WASM module..."
emcc "$CORE_LIB" \
  -O3 \
  -s WASM=1 \
  -s MODULARIZE=1 \
  -s EXPORT_ES6=1 \
  -s "EXPORT_NAME=create_gearcoleco" \
  -s ENVIRONMENT=node \
  ...
  -s INVOKE_RUN=0 \
  -s USE_ZLIB=1 \
  -o "$OUTPUT_DIR/gearcoleco_libretro.js"

echo "Built gearcoleco -> $OUTPUT_DIR/gearcoleco_libretro.js + .wasm"
echo "Done."
